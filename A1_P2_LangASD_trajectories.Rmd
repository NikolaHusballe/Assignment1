...
d= subset(data,VISIT == "1", select=c(CHI_MLU, Diagnosis, verbalIQ1, nonverbalIQ1))
M3 = lm(CHI_MLU ~ Diagnosis, d)
summary(M3)

ggplot(d, aes(Diagnosis, CHI_MLU))+
  geom_violin()

M4 = lm(verbalIQ1 ~ Diagnosis, d)
summary(M4)

ggplot(d, aes(Diagnosis, verbalIQ1))+
  geom_violin()

M5=lm(nonverbalIQ1 ~ Diagnosis,d)
summary(M5)

ggplot(d, aes(Diagnosis, nonverbalIQ1))+
  geom_violin()
...
```{r}
model1 = lmer(CHI_MLU ~ 1 + VISIT + Diagnosis + (1+VISIT|SUBJ), data, REML = F)
summary(model1)
```
...
```{r}
library(MuMIn)
null.model1 = lmer(CHI_MLU ~ 1 + VISIT + (1+ VISIT|SUBJ), data)

anova(null.model1, model1)

r.squaredGLMM(null.model1)
...
contrasts(data$VISIT) = contr.poly(6) 
...
model3 = lmer(MOT_MLU ~ 1 + VISIT + Diagnosis + (1 + VISIT | SUBJ), REML = FALSE, data)
summary(model3)

r.squaredGLMM(model3)

null.model3 = lmer(MOT_MLU ~ 1 + VISIT + (1+ VISIT|SUBJ), data)

anova(null.model3, model3)
```

visit (beta = 0.120, se = 0.018, t-value = 6.595, p < .05)
diagnosis (beta = 0.501, se = 0.113, t-value = 4.429, p < .05)
the full model explained 67,6 % of the variance
...
model4 = lmer(CHI_MLU ~ 1 + VISIT + verbalIQ1 + MOT_MLU + typesqqq_CHI + (1+VISIT|SUBJ), REML = F, data)
summary(model4)
